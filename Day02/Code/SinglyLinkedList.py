...
class SinglyLinkedList(object):
    ...
    def insert_before(self, node, value):
        ...
        if node == self.__head:  # 如果是在链表头之前插入数据节点，则直接插入
            ...
            return
        node_head = self.__head
        node_new = Node(value)
        not_found = False  # 如果在整个链表中都没有找到指定插入的Node节点，则该标记量设置为True
        while(node_head is not None) and (node_head.next_node != node):
            if node_head.next_node is None:  # 如果已经到了链表的最后一个节点，则表明该链表中没有找到指定插入的Node节点
                not_found = True
                break
            else:
                node_head = node_head.next_node
        if not not_found:
            node_new.next_node = node_head.next_node
            node_head.next_node = node_new
    ...
    def __reversed_self(self):
        ...
        while node is not None:
            pre, node = self.__reversed_with_tow_node(pre, node)

        self.__head.next = None
        self.__head = pre

    @staticmethod
    def __reversed_with_tow_node(pre, node):
        """翻转相邻两个节点.
        参数:
            pre:前一个节点
            node:当前节点
